...
def evaluator(gt_tensors, pred_tensors, metrics):
    ...
    for metric_module_name in metric_module_list:
        metric_module[metric_module_name] = getattr(__import__("src.utils.metrics_SUN", fromlist=[metric_module_name]),
                                                    metric_module_name)(length=len(gt_tensors))

    assert len(gt_tensors) == len(pred_tensors)

    for idx in range(len(gt_tensors)):
        pred_ten = torch.sigmoid(pred_tensors[idx])
        gt_ten = gt_tensors[idx]

        gt_ary = (gt_ten * 255).squeeze().data.cpu().numpy().astype(np.uint8)
        pred_ary = (pred_ten * 255).squeeze().data.cpu().numpy().astype(np.float32)
    ...
    for i, (name, value) in enumerate(res.items()):
    ...
    for metric_name in res.keys():
        ...
        if len(real_max_index[0]) > 0:
            case_score_list[metric_name] = max_case_score_list[real_max_index[0]].max().round(3)
        else:
            case_score_list[metric_name] = mean_case_score_list[real_mean_index[0]].mean().round(3)

    return case_score_list

if __name__ == "__main__":
    import torch
    gt, pred = torch.randn((8, 1, 256, 448)).cuda(), torch.randn((8, 1, 256, 448)).cuda()
    result = evaluator(gt, pred, metrics=["Smeasure", "meanEm", "wFmeasure", "meanFm", "maxDice", "meanSen"])
    print(result)