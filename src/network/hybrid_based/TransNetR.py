...
from src.network.hybrid_based.resnet import resnet50, resnet34
...
class TransNetR(nn.Module):
    def __init__(self, num_classes=1, input_hw=[256, 448], feature_return=False):
        super().__init__()
        ...
        self.r1 = residual_transformer_block(64+64, 64, dim=input_hw[1]//8*2)
        self.r2 = residual_transformer_block(64+64, 64, dim=input_hw[1])
...
if __name__ == "__main__":
    x = torch.randn((4, 3, 256, 448)).cuda()
    model = TransNetR().cuda()
    y = model(x)
    print(y.shape)